...
EOF
    ...
    # -----------------------------------------------------------------------
    # 6. Run Part 3 — Nginx proxy tests (MCP through HTTPS reverse proxy)
    # -----------------------------------------------------------------------
    echo ""
    info "=========================================="
    info "  Part 3 — Nginx proxy tests (HTTPS)"
    info "=========================================="
    echo ""
    # nginx is already running on port 443 from docker-compose.yml — no override needed.

    if MCP_NGINX_URL="https://localhost" \
       TEST_CLUSTER_HOST="$CLUSTER_HOST" \
       TEST_CONTEXT=docker \
       python -m pytest \
        "${TESTS_DIR}/test_nginx_proxy.py" \
        "${PYTEST_MARKER_ARGS[@]}" \
