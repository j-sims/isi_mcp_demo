...
- **Media and Entertainment**: Post-production workflows, video editing, rendering farms, content distribution
- **Healthcare and Life Sciences**: Medical imaging (PACS), genomics data, clinical research datasets, HIPAA-compliant archival
- **Financial Services**: Risk modeling, algorithmic trading data, regulatory compliance archives
- **AI/ML and GenAI**: Training data lakes, model checkpointing, GPU-accelerated inference pipelines
- **High-Performance Computing (HPC)**: Scientific simulation data, research datasets, parallel I/O workloads
- **General File Services**: Home directories, departmental shares, application data, backup targets

---

## Available MCP Tool Groups

The MCP server organizes tools into functional groups. Each group can be enabled or disabled at runtime. Use `powerscale_tools_list` to see current tool availability and `powerscale_tools_toggle` to enable/disable groups.

### Management Tools (Always Available)

| Tool | Purpose |
|---|---|
| `powerscale_tools_list` | List all tool groups and their enabled/disabled status |
| `powerscale_tools_toggle` | Enable or disable tool groups by name |
| `powerscale_cluster_list` | List all configured clusters and the currently selected cluster |
| `powerscale_cluster_select` | Switch the active target cluster |
| `powerscale_cluster_add` | Add a new cluster to the vault configuration |
| `powerscale_cluster_remove` | Remove a cluster from the vault configuration |

### Health and Capacity

| Tool | Purpose |
|---|---|
| `powerscale_check_health` | Comprehensive health check (quorum, service lights, critical events, network, capacity) |
| `powerscale_capacity` | Storage capacity statistics (used, available, total, dedupe/compression ratios) |
| `powerscale_config` | Cluster configuration details |

### Quota Management

| Tool | Purpose |
|---|---|
| `powerscale_quota_get` | List quotas with pagination |
| `powerscale_quota_create` | Create a new quota (directory, user, group) |
| `powerscale_quota_set` | Set quota limit to a specific size |
| `powerscale_quota_increment` | Increase a quota limit by a specified amount |
| `powerscale_quota_decrement` | Decrease a quota limit by a specified amount |
| `powerscale_quota_remove` | Remove a quota |

### Snapshot Management

| Tool | Purpose |
|---|---|
| `powerscale_snapshot_get` | List existing snapshots |
| `powerscale_snapshot_create` | Create a point-in-time snapshot |
| `powerscale_snapshot_delete` | Delete a snapshot |
| `powerscale_snapshot_schedule_get` | List snapshot schedules |
| `powerscale_snapshot_schedule_create` | Create a recurring snapshot schedule |
| `powerscale_snapshot_schedule_remove` | Remove a snapshot schedule |
| `powerscale_snapshot_pending_get` | List pending (scheduled) snapshots |
| `powerscale_snapshot_alias_create` | Create a snapshot alias |
| `powerscale_snapshot_alias_get` | Get snapshot alias details |

### SyncIQ Replication

| Tool | Purpose |
|---|---|
| `powerscale_synciq_get` | List SyncIQ replication policies |
| `powerscale_synciq_create` | Create a new replication policy |
| `powerscale_synciq_remove` | Remove a replication policy |

### DataMover

| Tool | Purpose |
|---|---|
| `powerscale_datamover_policy_get` | List DataMover policies |
| `powerscale_datamover_policy_get_by_id` | Get a specific DataMover policy |
| `powerscale_datamover_policy_create` | Create a DataMover policy |
| `powerscale_datamover_policy_delete` | Delete a DataMover policy |
| `powerscale_datamover_policy_last_job` | Get last job status for a policy |
| `powerscale_datamover_account_get` | List DataMover accounts |
| `powerscale_datamover_account_get_by_id` | Get a specific DataMover account |
| `powerscale_datamover_account_create` | Create a DataMover account |
| `powerscale_datamover_account_delete` | Delete a DataMover account |
| `powerscale_datamover_base_policy_get` | List DataMover base policies |
| `powerscale_datamover_base_policy_get_by_id` | Get a specific base policy |
| `powerscale_datamover_base_policy_create` | Create a DataMover base policy |
| `powerscale_datamover_base_policy_delete` | Delete a DataMover base policy |

### FilePool Policy Management

| Tool | Purpose |
|---|---|
| `powerscale_filepool_policy_get` | List FilePool policies |
| `powerscale_filepool_policy_get_by_name` | Get a specific FilePool policy |
| `powerscale_filepool_default_policy_get` | Get the default FilePool policy |
| `powerscale_filepool_policy_create` | Create a FilePool policy |
| `powerscale_filepool_policy_update` | Update a FilePool policy |
| `powerscale_filepool_policy_remove` | Remove a FilePool policy |

### NFS Export Management

| Tool | Purpose |
|---|---|
| `powerscale_nfs_get` | List NFS exports |
| `powerscale_nfs_create` | Create a new NFS export |
| `powerscale_nfs_remove` | Remove an NFS export |
| `powerscale_nfs_global_settings_get` | Get NFS global settings (versions, threads) |
| `powerscale_nfs_global_settings_set` | Modify NFS global settings |

### SMB Share Management

| Tool | Purpose |
|---|---|
| `powerscale_smb_get` | List SMB shares |
| `powerscale_smb_create` | Create a new SMB share |
| `powerscale_smb_remove` | Remove an SMB share |
| `powerscale_smb_global_settings_get` | Get SMB global settings (versions, encryption, signing) |
| `powerscale_smb_global_settings_set` | Modify SMB global settings |
| `powerscale_smb_sessions_get` | List active SMB sessions |
| `powerscale_smb_session_close` | Close a specific SMB session |
| `powerscale_smb_sessions_close_by_user` | Close all sessions for a user |
| `powerscale_smb_openfiles_get` | List open SMB files |
| `powerscale_smb_openfile_close` | Close a specific open file |

### S3 Bucket Management

| Tool | Purpose |
|---|---|
| `powerscale_s3_get` | List S3 buckets |
| `powerscale_s3_create` | Create a new S3 bucket |
| `powerscale_s3_remove` | Remove an S3 bucket |

### File and Directory Management

| Tool | Purpose |
|---|---|
| `powerscale_directory_list` | List directory contents |
| `powerscale_directory_attributes` | Get directory attributes |
| `powerscale_directory_create` | Create a directory |
| `powerscale_directory_delete` | Delete a directory |
| `powerscale_directory_move` | Move/rename a directory |
| `powerscale_directory_copy` | Copy a directory |
| `powerscale_file_read` | Read file contents (truncated at 1 MiB) |
| `powerscale_file_attributes` | Get file attributes |
| `powerscale_file_create` | Create a file |
| `powerscale_file_delete` | Delete a file |
| `powerscale_file_move` | Move/rename a file |
| `powerscale_file_copy` | Copy a file |
| `powerscale_acl_get` | Get ACL (access control list) for a path |
| `powerscale_acl_set` | Set ACL for a path |
| `powerscale_metadata_get` | Get metadata attributes |
| `powerscale_metadata_set` | Set metadata attributes |
| `powerscale_access_point_list` | List namespace access points |
| `powerscale_access_point_create` | Create a namespace access point |
| `powerscale_access_point_delete` | Delete a namespace access point |
| `powerscale_worm_get` | Get WORM/SmartLock properties |
| `powerscale_worm_set` | Set WORM/SmartLock properties |
| `powerscale_directory_query` | Query directories with filter conditions |

### User and Group Management

| Tool | Purpose |
|---|---|
| `powerscale_user_get` | List users |
| `powerscale_user_create` | Create a local user |
| `powerscale_user_modify` | Modify user properties |
| `powerscale_user_remove` | Remove a local user |
| `powerscale_group_get` | List groups |
| `powerscale_group_create` | Create a local group |
| `powerscale_group_modify` | Modify group membership |
| `powerscale_group_remove` | Remove a local group |

### Event Monitoring

| Tool | Purpose |
|---|---|
| `powerscale_event_get` | List event group occurrences with filtering |
| `powerscale_event_get_by_id` | Get a specific event occurrence by ID |

### Performance Statistics

| Tool | Purpose |
|---|---|
| `powerscale_stats_cpu` | CPU utilization statistics |
| `powerscale_stats_network` | Network throughput statistics |
| `powerscale_stats_disk` | Disk I/O statistics |
| `powerscale_stats_ifs` | /ifs file system statistics |
| `powerscale_stats_node` | Per-node resource statistics |
| `powerscale_stats_protocol` | Protocol operation statistics |
| `powerscale_stats_clients` | Connected client statistics |
| `powerscale_stats_get` | Get custom statistics by key names |
| `powerscale_stats_keys` | Discover available statistics keys |
...
| `bytes_to_human` | Convert bytes to human-readable format (IEC units) |
| `human_to_bytes` | Convert human-readable format to bytes |
